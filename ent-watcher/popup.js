...
async function refreshUI() {
  ...
  if ((status === "error" || status === "anomaly") && config.lastError) {
  ...
  if (!config.entUrl) {
    document.getElementById("checkNowBtn").title = "Configure d'abord l'URL de connexion CAS dans les réglages";
  }

  const alarmInfo = await chrome.runtime.sendMessage({ type: "GET_ALARM_INFO" });
  const alarm = alarmInfo?.alarm;
  const alarmStatus = document.getElementById("alarmStatus");
  const nextCheck = document.getElementById("nextCheck");
  if (alarm) {
    alarmStatus.textContent = "✅ Oui";
    alarmStatus.style.color = "#166534";
    nextCheck.textContent = formatDate(alarm.scheduledTime);
  } else {
    alarmStatus.textContent = "❌ Non";
    alarmStatus.style.color = "#991b1b";
